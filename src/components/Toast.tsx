...
// Hệ thống thông báo (toast) dùng chung - không phụ thuộc thư viện ngoài.
// Cách dùng:
//   const toast = useToast();
//   toast.success("Đăng nhập thành công", "Chào mừng bạn quay lại!");
//   toast.error("Có lỗi", "Email hoặc mật khẩu không đúng.");
//   toast.show({ variant: "info", title: "...", description: "...", duration: 6000 });
...
interface ToastOptions {
  ...
  /** Thời gian tự ẩn (ms). 0 = không tự ẩn. Mặc định 4500ms. */
  duration?: number;
}
...
const EXIT_MS = 260; // khớp với thời lượng animation rời đi

// Bảng màu theo từng loại, bám đúng palette của app.
const VARIANT = {
  success: {
    accent: "#4a6b50",
    chipBg: "#eef4ef",
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const dismiss = useCallback(
    (id: number) => {
      // Đánh dấu "leaving" để chạy animation rời đi, rồi mới gỡ khỏi DOM.
      setToasts((prev) =>
