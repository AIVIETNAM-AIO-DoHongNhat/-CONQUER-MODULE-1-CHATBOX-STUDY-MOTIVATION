...
// Lấy chữ cái đại diện cho avatar từ tên hoặc email.
...
export default function Header() {
  ...
  // Chỉ coi là đã đăng nhập sau khi mounted (tránh lệch hydration), có token và
  // profile không lỗi (nếu lỗi nghĩa là token hỏng/refresh thất bại).
  const loggedIn = mounted && !!getAccessToken() && !isError;

  // Đổi route thì đóng hết menu.
  useEffect(() => {
    setOpen(false);
    setMenuOpen(false);
  }, [pathname]);
    ...
    >
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5 sm:px-8">
        ...
        <div className="hidden items-center gap-2 md:flex">
          {/* Trước khi mounted: chừa chỗ trống để tránh nhảy layout + lệch hydration */}
          {!mounted ? (
